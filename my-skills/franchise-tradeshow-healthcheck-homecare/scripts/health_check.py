"""
Trade Show Online Health Check - FRANCHISE Home Care Edition

Fast (60-90 second) online presence audit for a franchise home care prospect.
...
Franchise version: no website audit (corporate-built), no overall letter grade.
...
Runs all checks in parallel:
  ...
  4. Reviews snapshot (Google from Places, FB/Yelp via search)
  5. Competitor ads check from each user-supplied city (parsed from SERP HTML)

Usage:
    python health_check.py "Visiting Angels" "Tampa" "FL" "Plant City" "St Petersburg"
...
# Two-letter particles that should only split if the remaining stub is
# a sensible length. Prevents weird splits on short domain stubs.
...
def humanize_domain(domain):
    ...
    # First split on hyphens/underscores
    pieces = [p for p in re.split(r"[-_]", sld) if p]
    # Then for each piece, greedy split from the back using the word list
    expanded = []
    for p in pieces:
        remaining = p.lower()
        found_any = True
        ...
        while remaining and found_any:
            ...
            for word in _DOMAIN_WORD_SPLITS:
                if not remaining.endswith(word):
                    continue
                # Allow exact match -- the peel will finish the string
                if len(remaining) < len(word):
                    continue
                # Skip short particles when the stub would be awkward
...
def build_ads_per_city(business, competitors, seo_per_city, prospect_website=""):
    ...
    for city, data in seo_per_city.items():
        ...
        # when the prospect's name is sitting in the ad block.
...
def run_health_check(business, cities, state):
    ...
    # Parallel: SEO (per city, also yields 3-Pack via local_pack), Reviews
    # No website audit in franchise version (corporate-built site)
    print(
        f"Running parallel scans (SEO+3-Pack, Reviews) "
    ...
    # Website audit skipped in franchise version (corporate-built site)
    ...
    # Individual grades only (no overall grade for franchise version)
    results["all_grades"] = {
    ...
    bar = "=" * 60
    print(bar)
    print(f"Scan complete    (scan time: {elapsed}s)")
    print(bar)
    print()
    sys.stdout.flush()
