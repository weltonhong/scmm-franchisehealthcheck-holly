...
# Rep block printed on every page of the CTA footer. Each franchise
# deploy repo overrides REP_NAME, REP_EMAIL, and CALENDLY_URL above to
# match its sales rep. BRAND_LINE stays the same across SCMM deploys.
...
def _build_recommendations_top2(results):
    ...
    # Website recommendations removed in franchise version
...
def build_pdf(results):
    ...
    fname = f"FranchiseHealthCheck_{safe_filename(business)}_{datetime.date.today().isoformat()}.pdf"
    ...
    # Hook -- use the worst individual grade to pick the tone
    worst_grades = [g for g in grades.values() if g]
    worst = max(worst_grades, key=lambda g: {"F": 4, "D": 3, "C": 2, "B": 1, "A": 0}.get(g, 0)) if worst_grades else "C"
    low, high = estimate_lost_calls(results)
    hook_text = build_adaptive_hook(results, worst, low, high)
    ...
    # Market Snapshot - one-line competitive context
    ...
    # No overall grade banner in franchise version
    ...
    # Website section removed in franchise version (corporate-built site)
