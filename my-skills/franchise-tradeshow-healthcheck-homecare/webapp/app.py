...
When the scan finishes, the scorecard renders with individual section grades
...
if st.session_state.get("scan_started") and not st.session_state.get("scan_done"):
    ...
    if missing:
        ...
        log_lines = []
        spinner_box.info("Scanning... reviews, 3-Pack, SEO, ads running in parallel.")
...
if st.session_state.get("scan_done"):
    ...
    if results:
        ...
        rows_html = []
        for key in ("reviews", "3pack", "seo", "ads"):
            g = grades.get(key)
            badge = (
        ...
        if pdf_bytes:
            st.download_button(
                label="📄 Download PDF",
                data=pdf_bytes,
                file_name=st.session_state.get("pdf_filename", "FranchiseHealthCheck.pdf"),
